  ...
  <div class="box">
    ...
    <div class="box-main">
      ...
      <el-dialog title="任务详情" :visible.sync="dialogVisible" width="70%">
        ...
        <span>任务名称：</span>
        <el-input class="addInput" size="small" v-model="addinput" placeholder="请输入任务名称"></el-input>
        <el-form class="form">
      ...
      <!-- 新增对话框的巡检配置 -->
      <el-dialog title="选择巡查对象" :visible.sync="dialogVisible1" width="60%">
        <!-- <span>这是一段信息</span> -->
        <el-button size="small" class="roleRight" type="primary" @click="getTypeName">查询</el-button>
        <el-input size="small" v-model="duixianginput" class="roleInput" placeholder="请输入检查对象名称"></el-input>
        <template>
          <el-select
            class="roleRight"
            size="small"
      ...
      <el-dialog title="任务详情" :visible.sync="dialogVisible3" width="70%">
        <span class="star">*&nbsp</span>
        <span>任务名称：</span>
        <el-input class="addInput" size="small" v-model="amendinput" placeholder="请输入任务名称"></el-input>
        <el-form class="form">
          <el-form-item class="eightelform" label="选择频率:" required>
            <template>
        ...
        <div class="role">
          ...
          <el-input v-model="amendroleinput" size="small" class="roleInput" placeholder="请输入人员名称"></el-input>
          <template>
            <el-select
              class="roleRight"
      ...
      <!-- 编辑对话框中选择巡查对象 -->
      <el-dialog title="选择巡查对象" :visible.sync="dialogVisible4" width="60%">
        <!-- <span>这是一段信息</span> -->
        <el-button size="small" class="roleRight" type="primary" @click="amendGetName">查询</el-button>
        <el-input
          size="small"
          v-model="amendduixianginput"
          class="roleInput"
...
export default {
  data() {
    return {
      ...
      deptId: '', // 进入页面保存用户cookie中的机构id
      addoptionsArea: [], // 进入页面渲染所有园区
      addArea: '',
  ...
  methods: {
    ...
    getAllInfo() {
      ...
      this.addDanger
        .get('/devicerountingplan/getDeviceRountingPlanByTime', {
          params: this.obj
        })
        .then(res => {
          // console.log(res)
    ...
    // 查询巡检设备
    getDeviceInfo() {
      this.xunjianObj.pageNo = this.currentPage1
      this.xunjianObj.pageSize = this.pageSize
      return this.addDanger
        .get('/deviceTrountQrcode/getDeviceTrountQrcodefoList', {
          params: this.xunjianObj
        })
        .then(res => {
          // console.log(res.data.data)
          res.data.data.rows.forEach(v => {
    ...
    // 切换新增对话框中的园区选择下拉菜单实现筛选
    changeArea(value) {
      console.log(value)
      this.addPlan.parkid = value
      this.xunjianObj.parkid = value
      this.getDeviceInfo()
    },
    // 切换编辑对话中的园区进行筛选
    ...
    async addPlanOk() {
      ...
      await this.$request.plan.addPlan(this.addPlan)
      await this.$message({
        type: 'success',
        message: '新增成功',
        duration: 1000
    ...
    async deleteInfo(row) {
      //   console.log(row)
      try {
        await this.$confirm('你确定要删除该巡检计划吗？', '温馨提示', {
          type: 'warning'
        })
        const res = await this.$request.plan.deleteInfo({
          id: row.id
        })
        console.log(res)
        this.$message({
    ...
    async amendInfo(row) {
      ...
      // 根据巡检计划id获取巡检配置
      // const res = await this.$request.plan.getPlanInfo({
      //   id: row.id
      // })
      const res = await this.addDanger.get(
        '/devicerountingplan/selectRountingPlan',
        {
          params: {
            id: row.id
          }
    ...
    async changeType(value) {
      this.xunjianObj.otherOption = value
      this.getDeviceInfo()
      // const res = await this.addDanger.get(
    ...
    // 根据新增巡检名称筛选
    async getTypeName() {
      this.xunjianObj.planName = this.duixianginput
      this.getDeviceInfo()
      //   const res = await this.addDanger.get(
    ...
    async amendPlanBtn() {
      ...
      // for (let key in this.amendPlan) {
      //   // console.log(this.amendPlan[key])
      //   if (this.amendPlan[key] === '') {
      //     this.$message({
      //       type: 'error',
      //       message: '请将信息填写完整',
      //       duration: 1000
      ...
      //   }
      // }
      const res = await this.$request.plan.amendPlanInfo(this.amendPlan)
      console.log(res)
      await this.$message({
        type: 'success',
        message: '修改成功',
