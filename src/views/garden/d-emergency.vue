          ...
          >
            ...
            <el-table-column label="当前状态">
              <template v-slot="{row}">
                ...
                <span
                  :class="row.flag? 'noOrder' : 'overAll'"
                  @click="noOrder(row)"
                >{{institutionType === '1'? '--': row.order}}</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getDabgerInfo() {
      ...
      this.addDanger
        .get('/warn/selectTaskInfoListByWb', {
    ...
    async lookPicture() {
      ...
      if (this.examineObj.three.indexOf('.') != -1) {
        ...
        // 添加随机数防止浏览器缓存
        const res = await this.addDanger.get('/taskwarn/getiotpictureUrl', {
          params: this.picObj
        })
        console.log(res.data.data)
